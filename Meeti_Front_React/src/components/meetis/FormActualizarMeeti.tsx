...
import {findAllGruposByUsuarioId, findMeetiByIdPeticion, updateMeetiPeticion} from "../../api/ApiSpringBoot";
import Cargando from "../Cargando";
import Error404 from "../error/404";
import type {Grupo, Meeti, SaveMeeti} from "../../types";
...
const FormActualizarMeeti = () => {
    ...
    const {register, handleSubmit, formState: {errors}, reset, setValue} = useForm<SaveMeeti>();
    const {data, isLoading, isError} = useQuery<Meeti>({
        queryKey: ["findMeetiById", id],
        queryFn: () => findMeetiByIdPeticion(id),
        retry: false,
    ...
    useEffect(() => {
        if (data) {
            let horaFormateada = "";
            if (data.hora.includes(":")) {
                horaFormateada = data.hora.substring(0,5); // HH:mm
            } else {
                const horaObj = new Date(data.hora);
                horaFormateada = `${String(horaObj.getHours()).padStart(2, '0')}:${String(horaObj.getMinutes()).padStart(2, '0')}`;
            }
    ...
    function actualizacionMeeti(data) {
        ...
        updateMeetiMutation.mutate(data);
    }

    const updateMeetiMutation = useMutation({
        mutationKey: ["updateMeetiById"],
        mutationFn: updateMeetiPeticion,
        onError: () => {
            toast.error("Ocurrio un error en la actualizacion del meeti")
            navigate("/administracion/")
        },
        onSuccess: () => {
            toast.success("Meeti actualizado correctamente");
            ...
            <div className="max-w-4xl mx-auto">
                <div className="bg-white p-5 py-10 border rounded-lg shadow-md">
                    <h2 className="text-center font-fjalla text-3xl border-b-2 border-red-600 pb-3">
                        Actualizar Meeti - {data.titulo}
                    </h2>

                    <form className="space-y-5"
                          onSubmit={handleSubmit(actualizacionMeeti)}
                    >
                        ...
                        {/*Grupo*/}
                        <div className="mt-4">
                            <label className="font-fjalla font-semibold text-gray-600 text-xl block mb-2"
                                   htmlFor="grupo">
                                Grupo
                            </label>
                            <select
...
export default FormActualizarMeeti;